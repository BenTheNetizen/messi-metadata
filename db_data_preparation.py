...
olympic["Name"] = (
    olympic["Name"].str.split(" ").str[0] + " " + olympic["Name"].str.split(" ").str[-1]
)
...
player_stats = (
    appearance.groupby(["player_id", "year"])
    .agg(
        {
...
player_stats = player_stats.merge(
    df, how="left", left_on="player_pretty_name", right_on="Name"
)
player_stats = player_stats.drop("Name", axis=1)

# Handle players with multiple olympic medals
p3 = player_stats.groupby("player_id").agg(
    {
        "player_id": "first",
        "player_pretty_name": "first",
...
player_stats = player_stats.rename(
    columns={"player_id_x": "player_id", "olympic_y": "olympic"}
)
player_stats = player_stats.drop_duplicates(subset=["player_id", "year"])


# Prepare club_stats
club_stats = (
    appearance.groupby(["player_club_id", "year"])
    .agg(
        {
            "yellow_cards": "sum",
            "red_cards": "sum",
...
club_stats = club_stats.merge(
    club[["club_id", "pretty_name"]],
    how="left",
